...
function AddCarPage() {
  ...
  return (
    <div className="min-h-screen flex bg-white dark:bg-slate-950 transition-colors duration-300 font-sans">
        ...
        <div className="max-w-xl w-full mx-auto">
          ...
          <div className="mb-10">
            <div className="inline-flex items-center justify-center p-3 bg-indigo-50 dark:bg-indigo-900/30 rounded-2xl mb-6">
              <DocumentPlusIcon className="w-8 h-8 text-indigo-600 dark:text-indigo-400" />
            </div>
            <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight mb-2">Register Vehicle</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex-1">
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <BeakerIcon className="h-5 w-5 text-slate-400 dark:text-slate-500" />
                  </div>
                  <input type="text" name="brand" placeholder="Toyota" value={carData.brand} onChange={handleChange} required
                    className="block w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900 border-0 text-slate-900 dark:text-white rounded-xl ring-1 ring-inset ring-slate-200 dark:ring-slate-800 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-800 transition-all sm:text-sm sm:leading-6 placeholder:text-slate-400 dark:placeholder:text-slate-600" />
                </div>
              </div>
              <div className="flex-1">
                ...
                <input type="text" name="model" placeholder="Axio" value={carData.model} onChange={handleChange} required
                  className="block w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-900 border-0 text-slate-900 dark:text-white rounded-xl ring-1 ring-inset ring-slate-200 dark:ring-slate-800 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-800 transition-all sm:text-sm sm:leading-6 placeholder:text-slate-400 dark:placeholder:text-slate-600" />
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              ...
              <div className="flex-1">
                ...
                <select name="fuelType" value={carData.fuelType} onChange={handleChange} required
                  className="block w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-900 border-0 text-slate-900 dark:text-white rounded-xl ring-1 ring-inset ring-slate-200 dark:ring-slate-800 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-800 transition-all sm:text-sm sm:leading-6">
            ...
            <div>
              ...
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <CurrencyDollarIcon className="h-6 w-6 text-slate-400 dark:text-slate-500" />
                </div>
                <input type="number" name="pricePerDay" placeholder="8500" value={carData.pricePerDay} onChange={handleChange} required
                  className="block w-full pl-12 pr-4 py-4 bg-slate-50 dark:bg-slate-900 border-0 text-slate-900 dark:text-white font-bold text-lg rounded-xl ring-1 ring-inset ring-slate-200 dark:ring-slate-800 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:focus:ring-indigo-500 focus:bg-white dark:focus:bg-slate-800 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600" />
              </div>
            </div>

            <div className="pt-4">
              <button type="submit" className="w-full flex items-center justify-center gap-2 bg-slate-900 dark:bg-indigo-600 hover:bg-indigo-600 dark:hover:bg-indigo-500 text-white font-bold py-4 px-8 rounded-xl shadow-lg hover:shadow-indigo-200 dark:hover:shadow-indigo-900 transform hover:-translate-y-1 transition-all duration-300">
      ...
      <div className="hidden lg:flex w-2/5 relative bg-slate-900 flex-col justify-center items-center p-12 overflow-hidden border-l border-slate-800">
        ...
        <div className="relative z-10 w-full max-w-sm">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-2xl transform transition-all duration-500 hover:scale-[1.02] border border-slate-100 dark:border-slate-700">
            <div className="h-56 bg-slate-100 dark:bg-slate-700 relative group flex items-center justify-center">
              {carData.imageUrl && carData.imageUrl !== 'null' && carData.imageUrl !== '' ? (
              ...
              {carData.fuelType && (
                <div className="absolute top-4 right-4 bg-slate-900/90 dark:bg-indigo-600/90 backdrop-blur-sm text-white text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-wider border border-white/10 shadow-lg">
                  {carData.fuelType}
                </div>
              )}
            </div>
            ...
            <div className="p-6">
              <div className="mb-2">
                <h3 className="text-2xl font-black text-slate-900 dark:text-white leading-tight">
                  {carData.brand || 'Brand'} <span className="text-slate-500 dark:text-slate-400 font-medium">{carData.model || 'Model'}</span>
                </h3>
              </div>

              <div className="flex items-end justify-between mt-6">
                <div>
                  <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Daily Rate</p>
                  <p className="text-indigo-600 dark:text-indigo-400 font-black text-xl">
                    LKR {carData.pricePerDay || '0'}
                  </p>
                </div>
                <div className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-700 px-3 py-1.5 rounded-lg text-slate-600 dark:text-slate-300 border border-slate-100 dark:border-slate-600">
